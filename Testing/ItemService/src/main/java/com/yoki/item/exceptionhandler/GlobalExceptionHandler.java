package com.yoki.item.exceptionhandler;
...
public class GlobalExceptionHandler {
	...
	File errorFile= new File("src/logger/"+currentDate+"-"+"ItemService"+" - Exception"+".txt");
	private ErrorDetails logException(Exception ex, WebRequest request) {
		//log here
		return new ErrorDetails(LocalDateTime.now(),ex.getMessage(),request.getDescription(false),null);
	}
	...
	ErrorDetails errorDetails=new ErrorDetails(LocalDateTime.now(),ex.getMessage(),request.getMethod(),request.getRequestURI());
	...
	public boolean writeJsExceptionFile(Exception ex,String jsMethod,HttpServletRequest request) throws IOException
	{
	boolean result=false;
	try(PrintWriter writer=new PrintWriter(new FileWriter(errorFile,true)))
	{
	
	ErrorDetails errorDetails=new ErrorDetails(LocalDateTime.now(),ex.getMessage(),jsMethod,request.getRequestURI());
